...
def get_neuronID():
    with open('/data2/liaominghui/project/HemiBrain/data/manc_type/manc_type_info/neuron2ID.txt', 'r') as file:
        data = {}
        for line in file:
...
class MancSkeleton_type(InMemoryDataset):
    ...
    def process_set(self):
        # to create BrianData using raw file and Data type
    ...
    def __repr__(self) -> str:
        # print the BrainData type info for print(BrainData)
        return f'{self.__type__.__name__}({len(self)})'


if __name__ == '__main__':
    path = 'data/manc_type'
    pre_transform, transform = T.NormalizeScale(), T.FixedPoints(1024)
    data_dataset = MancSkeleton_type(path, 'data', transform=transform, pre_transform=pre_transform)
    pass
