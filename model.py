...
from dataset_util.HemiSkeleton import HemiSkeleton, HemiSkeletonMask
...
class PointNetSkeleton_node(torch.nn.Module):
    def __init__(self, dataset_num_features, represent_features, num_class, num_PointNet2Layer=3):
        super().__init__()
        assert represent_features <= dataset_num_features
        # Input channels account for both `pos` and node features.
        self.sa1_module = SAModule(0.5, 0.2, MLP([3, 64, 64, 256]))
